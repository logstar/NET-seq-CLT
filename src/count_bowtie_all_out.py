...
def parse_bt_file(in_bt_fn, qcutoff, ref_length):
    ...
    return [bt_align_counter, num_all_aligned, num_qual_failed]

def write_parse_bt_stats(stats_list, stats_fn):
    num_all_aligned = stats_list[0]
    num_qual_failed = stats_list[1]
    num_filtered = num_all_aligned - num_qual_failed

    stats_file = open(stats_fn, 'w')
    stats_file.write("Number of all alignments:%d\n" % num_all_aligned)
    stats_file.write("Number of reads with quality socre <= cutoff: %d (%s%%)\n" 
                      % (num_qual_failed, "{:.4f}".format(float(num_qual_failed) / num_all_aligned * 100)))
    stats_file.write("Number of all alignments passed quality socre filter: %d (%s%%)\n" 
                      % (num_filtered, "{:.4f}".format(float(num_filtered) / num_all_aligned * 100)))
...
<input bowtie output (allow alignment to multiple locations)> \
<output stats file> <output start position count file 0 based>\n' % argv[0])
        return -2
